...
for (let i = 0; i < tabBtnYosos.length; i++) {
  tabBtnYosos[i].addEventListener("click", function () {
    for (let j = 0; j < tabBtnYosos.length; j++) {
      tabBtnYosos[j].classList.remove("isActive");
    }

    for (let k = 0; k < tabPanelYosos.length; k++) {
      tabPanelYosos[k].classList.remove("isActive");
    }
