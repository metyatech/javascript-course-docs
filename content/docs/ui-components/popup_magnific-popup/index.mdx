...
<Hint>
「ポップアップ/ダイアログ/モーダル (Magnific Popup) / 演習1（テキストリンクから画像ポップアップ）」では、問題文の「テキストのリンクをクリックすると、画像がポップアップで表示されるものを作成して下さい」を満たすために、ポップアップ対象のセレクタと設定項目を先に分けて考えましょう。特に `<a>`、`href`、`class="image-link"`、`$('.image-link')` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「ポップアップ/ダイアログ/モーダル (Magnific Popup) / 演習2（画像クリックでポップアップ）」では、問題文の「サムネイル画像をクリックすると、同じ画像が大きくポップアップで表示されるものを作成して下…」を満たすために、ポップアップ対象のセレクタと設定項目を先に分けて考えましょう。特に `<a>`、`href`、`<img src>`、`width="200"` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「ポップアップ/ダイアログ/モーダル (Magnific Popup) / 演習3（テキストコンテンツのポップアップ）」では、問題文の「リンクをクリックすると、テキストがポップアップで表示されるものを作成して下さい」を満たすために、ポップアップ対象のセレクタと設定項目を先に分けて考えましょう。特に `href="#content"`、`#`、`<div id="content">`、`class="mfp-hide"` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「ポップアップ/ダイアログ/モーダル (Magnific Popup) / 演習-発展1（モーダルウィンドウ風のポップアップ）」では、問題文の「リンクをクリックすると、見た目を整えたモーダルウィンドウ風のポップアップが表示されるもの…」を満たすために、ポップアップ対象のセレクタと設定項目を先に分けて考えましょう。特に `class="mfp-hide white-popup"`、`mfp-hide`、`white-popup`、`#modal-content` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「ポップアップ/ダイアログ/モーダル (Magnific Popup) / 演習-発展2（YouTube動画のポップアップ）」では、問題文の「リンクをクリックすると、YouTube動画がポップアップで表示されるものを作成して下さい」を満たすために、ポップアップ対象のセレクタと設定項目を先に分けて考えましょう。特に `href`、`type: 'iframe'`、`https://www.youtube.com/watch?v=dQw4w9WgXcQ`、`https://www.youtube.com/embed/dQw4w9WgXcQ` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「ポップアップ/ダイアログ/モーダル (Magnific Popup) / 演習-発展3（画像ギャラリー）」では、問題文の「複数の画像をギャラリーとして表示し、ポップアップ内で左右の矢印で画像を切り替えられるよう…」を満たすために、ポップアップ対象のセレクタと設定項目を先に分けて考えましょう。特に `<div class="gallery">`、`<a>`、`href`、`delegate: 'a'` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
