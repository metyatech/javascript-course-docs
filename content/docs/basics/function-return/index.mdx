...
<Hint>
「関数（戻り値） / 演習1（2 乗を返す関数）」では、問題文の「整数 n を受け取り、その 2 乗（n × n）を戻り値として返す関数 square(n…」を満たすために、`return` で関数の外へ返す値を先に分けて考えましょう。特に `square(n)`、`square(4)`、`square(5)`、`square` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「関数（戻り値） / 演習1-発展（合計を返す）」では、問題文の「1 から n までの和を計算し、その値を戻り値として返す関数 sum(n) を作り、su…」を満たすために、`for` の開始値・続ける条件・1回ごとの変化を先に分けて考えましょう。特に `sum(n)`、`sum(10)`、`sum`、`goukei` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「関数（戻り値） / 演習2（最小値）」では、問題文の「2 つの数のうち小さい方を戻り値とする関数 min(a, b) を作れ」を満たすために、`if` の条件式と、条件が成り立ったときの処理を先に分けて考えましょう。特に `min(a, b)`、`min(5, 9)`、`min(8, 4)`、`min` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「関数（戻り値） / 演習2-発展1（偶奇判定の文字列）」では、問題文の「整数 n を受け取り、偶数なら "偶数"、奇数なら "奇数" という文字列を戻り値として…」を満たすために、`if` の条件式と、条件が成り立ったときの処理を先に分けて考えましょう。特に `parity(n)`、`parity(10)`、`parity(7)`、`parity` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「関数（戻り値） / 演習2-発展2（最大値）」では、問題文の「3 つの整数 a, b, c の最大値を戻り値として返す関数 max3(a, b, c)…」を満たすために、`if` の条件式と、条件が成り立ったときの処理を先に分けて考えましょう。特に `max3(a, b, c)`、`max3(3, 8, 5)`、`max3`、`saidaichi` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「関数（戻り値） / 演習2-発展3（FizzBuzz の 1 行判定）」では、問題文の「整数 n を受け取り、次のルールに従って 1 つの文字列を戻り値として返す関数 fizz…」を満たすために、`if` / `else if` の判定順を先に分けて考えましょう。特に `fizzbuzz(n)`、`1, 3, 5, 15, 16`、`fizzbuzz`、`.log` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
