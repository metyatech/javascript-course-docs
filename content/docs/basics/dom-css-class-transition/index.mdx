...
<Hint>
「クラス切り替えでCSSアニメーション / 演習1（transitionなしでクラス切り替え）」では、問題文の「次のプレビューのように動作するよう、HTML/CSS/JavaScriptを書け（幅や色…」を満たすために、問題文の条件と完成例で変わる値を先に分けて考えましょう。特に `active` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「クラス切り替えでCSSアニメーション / 演習2（滑らかになるようにアニメーション化）」では、問題文の「演習1と同じように動作するが、次のプレビューのように、さらにサイズや色の変化が滑らかに見…」を満たすために、問題文の条件と完成例で変わる値を先に分けて考えましょう。特に `active` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「クラス切り替えでCSSアニメーション / 演習3（hoverで影をつける）」では、問題文の「次のように動作するよう、CSSとJavaScriptを書け（幅や色は自由で良い）」を満たすために、問題文の条件と完成例で変わる値を先に分けて考えましょう。完成例の中で、問題文の条件に対応している行を先に見つけましょう。
</Hint>
...
### 演習4（ON/OFF表示をclassList.containsで切り替える）
...
<Hint>
「クラス切り替えでCSSアニメーション / 演習4（ON/OFF表示をclassList.containsで切り替える）」では、問題文の「次のプレビューのように動作するよう、HTML/CSS/JavaScriptを書け（幅や色…」を満たすために、問題文の条件と完成例で変わる値を先に分けて考えましょう。特に `classList.contains` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「クラス切り替えでCSSアニメーション / 演習-発展1（お気に入りボタン）」では、問題文の「次のプレビューのように動作する、お気に入りボタンを作成せよ」を満たすために、付け外しするクラス名と、切り替えるタイミングを先に分けて考えましょう。特に `active`、`transition: color 0.3s;`、`classList.contains`、`.contains` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「クラス切り替えでCSSアニメーション / 演習-発展2（通知バナー）」では、問題文の「次のプレビューのように動作する、通知バナーを作成せよ」を満たすために、問題文の条件と完成例で変わる値を先に分けて考えましょう。特に `display: none;`、`show`、`display: block;`、`disabled` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「クラス切り替えでCSSアニメーション / 演習-発展3（まずは単純なカード選択）」では、問題文の「最初は、 単一の for...of だけ を使ったシンプルな例に挑戦します」を満たすために、取得するHTML要素と、書き換える表示内容を先に分けて考えましょう。特に `selected`、`querySelectorAll(".card")`、`for...of`、`.card` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「クラス切り替えでCSSアニメーション / 演習-発展4（単一 for-of でカード選択＋カウント）」では、問題文の「次に、 単一の for...of の中で完結する 少しだけ発展したパターンを練習します」を満たすために、配列から1つずつ取り出す変数と出力順を先に分けて考えましょう。特に `selected`、`for...of`、`.of` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「クラス切り替えでCSSアニメーション / 演習-発展5（アコーディオン）」では、問題文の「次のプレビューのように動作する、アコーディオンUIを作成せよ」を満たすために、取得するHTML要素と、書き換える表示内容を先に分けて考えましょう。特に `display: none;`、`display: block;`、`querySelectorAll(".question")`、`querySelectorAll(".answer")` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
