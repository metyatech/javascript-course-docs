...
<Hint>
「オブジェクト入門 / 演習1」では、問題文の「1. 自分の情報をまとめたオブジェクトを作れ」を満たすために、`console.log` に渡す値と出力順を先に分けて考えましょう。特に `{ ... }`、`name`、`age`、`オブジェクト.キー` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「オブジェクト入門 / 演習1-発展1」では、問題文の「次のexampleオブジェクトの中の奥深くにあるstrプロパティの値にアクセスするプログ…」を満たすために、`console.log` に渡す値と出力順を先に分けて考えましょう。特に `example`、`obj`、`example.obj`、`str` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「オブジェクト入門 / 演習1-発展2」では、問題文の「1. 次のdogオブジェクトのメソッドgreetを呼び出せ」を満たすために、`for` の開始値・続ける条件・1回ごとの変化を先に分けて考えましょう。特に `${dog.name}です。`、`greet`、`dog.name`、`bark(numberOfBarks)` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
