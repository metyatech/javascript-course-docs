...
<Hint>
「配列のメソッド / 演習1（push / pop 基本）」では、問題文の「次の配列 fruits を使い、下の手順を上から順に正確に実行し、各手順で指示された通り…」を満たすために、配列へ追加する値と追加後の確認を先に分けて考えましょう。特に `fruits`、`console.log(fruits);`、`console.log("取り出されたもの: " + 値);`、`console.log("残りの配列: " + fruits);` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「配列のメソッド / 演習2（unshift / shift 基本）」では、問題文の「次の配列 colors を使い、下の手順を上から順に実行し各手順で指示された通りに出力し…」を満たすために、`console.log` に渡す値と出力順を先に分けて考えましょう。特に `colors`、`console.log("取り出されたもの: " + 値);`、`removed`、`.unshift` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
let twiceArray = array.map(function (value) {
    return value * 2;
});
...
<Hint>
「配列のメソッド / 演習3（各要素に1を足す）」では、問題文の「次の開始配列の各要素に 1 を足した配列を作り、出力せよ」を満たすために、配列の各要素を別の値へ変換する処理を先に分けて考えましょう。特に `array`、`result`、`.map`、`.log` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「配列のメソッド / 演習4（変換）」では、問題文の「次の開始配列を、文字列 "No.値" の配列へ変換して出力せよ」を満たすために、配列の各要素を別の値へ変換する処理を先に分けて考えましょう。特に `array`、`result`、`.map`、`.log` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「配列のメソッド / 発展演習1（配列から要素を取り出して結合）」では、問題文の「次の配列を使い、array1 から "B" と "C" を取り出して array2 の末…」を満たすために、`console.log` に渡す値と出力順を先に分けて考えましょう。特に `array1`、`array2`、`removed`、`.splice` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「配列のメソッド / 発展演習2（join で文字列にする）」では、問題文の「次の配列を使って、要素を半角スペース区切りでつなげた文字列を1行で出力せよ」を満たすために、`console.log` に渡す値と出力順を先に分けて考えましょう。特に `console.log(arrayA.join(" "));`、`arrayA`、`.log`、`.join` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「配列のメソッド / 発展演習3（数値を昇順にソート）」では、問題文の「次の配列を数値の昇順にソートして console.log(arrayB); で出力しなさ…」を満たすために、並べ替えで比較する値を先に分けて考えましょう。特に `console.log(arrayB);`、`sort`、`arrayB`、`.sort` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「配列のメソッド / 発展演習4（条件位置以降を fill で埋める）」では、問題文の「次の配列から値 25 の位置を見つけ、その位置より後ろ（25 自身は含めない）をすべて …」を満たすために、`console.log` に渡す値と出力順を先に分けて考えましょう。特に `console.log(arrayC);`、`arrayC`、`pos`、`.indexOf` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「配列のメソッド / 発展演習5（ソートとフィルター）」では、問題文の「1. 次の items を価格 price の昇順でソートし、ソート結果を次のように 1…」を満たすために、残したい配列要素の条件を先に分けて考えましょう。特に `items`、`drinks`、`sum`、`avg` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
let drinks = items.filter(function (it) {
    return it.type === '飲料';
});
let sum = drinks.reduce(function (acc, it) {
    return acc + it.price;
}, 0);
...
<Hint>
「配列のメソッド / 発展演習6（検索）」では、問題文の「1. 発展演習5の items に対して、価格が 100 以上 の最初の 飲料 を検索し…」を満たすために、`return` で関数の外へ返す値を先に分けて考えましょう。特に `items`、`idx`、`found`、`.findIndex` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
let idx = items.findIndex(function (it) {
    return it.type === '飲料' && it.price >= 100;
});
let found = items[idx];
