...
<Hint>
「配列入門 / 演習1」では、問題文の「配列の要素を for...of 文で先頭から順に取り出し、「日」「月」…「土」と出力せよ」を満たすために、配列から1つずつ取り出す変数と出力順を先に分けて考えましょう。特に `for (let day of arr)`、`console.log(day)`、`day`、`arr` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「配列入門 / 演習2」では、問題文の「次の配列を使って、「日曜日」「月曜日」…「土曜日」と出力せよ」を満たすために、配列から1つずつ取り出す変数と出力順を先に分けて考えましょう。特に `for...of`、`for (let day of arr)`、`day`、`"日" + "曜日"` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「配列入門 / 演習2-発展1」では、問題文の「配列のすべての値の合計と平均を計算して出力せよ」を満たすために、配列から1つずつ取り出す変数と出力順を先に分けて考えましょう。特に `sum`、`for...of`、`sum += num;`、`avg = sum / arr.length` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「配列入門 / 演習2-発展2」では、問題文の「次の配列の値を逆順に（後ろから）出力せよ」を満たすために、`for` の開始値・続ける条件・1回ごとの変化を先に分けて考えましょう。特に `arr.length - 1`、`for (let i = arr.length - 1; i >= 0; i--)`、`i`、`arr[i]` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「配列入門 / 演習2-発展3」では、問題文の「次の配列の最小値を for...of 文を使って求めて出力せよ」を満たすために、`if` の条件式と、条件が成り立ったときの処理を先に分けて考えましょう。特に `for...of`、`min`、`num`、`if (num < min)` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「配列入門 / 演習2-発展4（1）」では、問題文の「次の配列をその下の図のように出力せよ」を満たすために、配列から1つずつ取り出す変数と出力順を先に分けて考えましょう。特に `|`、`tetrisField`、`row`、`line` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
...
<Hint>
「配列入門 / 演習2-発展4（2）」では、問題文の「（1）の tetrisField はテトリスというパズルゲームのフィールドを表している」を満たすために、`if` の条件式と、条件が成り立ったときの処理を先に分けて考えましょう。特に `isFilledRow(row)`、`for...of`、`popAlignedRows`、`popRow` がどの値・要素を表すか確認してから書き始めましょう。
</Hint>
